...
export default function TrainingSidebar({ 
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="px-2.5 py-2 border-b border-ink-light/30 shrink-0">
        <div className="flex items-center gap-1.5 mb-1.5">
          <span className="text-[10px] text-gold-dim uppercase tracking-wider font-semibold">⭐ Favourites</span>
          <span className="text-[9px] text-mist-dark">({selectedTechniques.length})</span>
        </div>
        {selectedTechniques.length === 0 ? (
          <p className="text-[10px] text-mist-dark italic py-1">Select techniques to add to favourites</p>
        ) : (
          <div className={`${isCompact ? 'space-y-0.5' : 'space-y-1'} max-h-40 overflow-y-auto sidebar-scroll`}>
            {selectedTechniques
